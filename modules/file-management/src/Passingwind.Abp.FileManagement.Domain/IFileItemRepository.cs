...
public interface IFileItemRepository : IRepository<FileItem, Guid>
{
    ...
    Task<bool> IsFileNameExistsAsync(Guid containerId, string fileName, Guid? parentId = null, bool? isDirectory = null, CancellationToken cancellationToken = default);

    Task<FileItem?> FindByNameAsync(Guid containerId, string fileName, Guid? parentId = null, bool? isDirectory = null, CancellationToken cancellationToken = default);

    Task<FileItem> GetByNameAsync(Guid containerId, string fileName, Guid? parentId = null, bool? isDirectory = null, CancellationToken cancellationToken = default);

    Task<FileItem?> FindByPathAsync(Guid containerId, string filePath, CancellationToken cancellationToken = default);

    Task<FileItem> GetByPathAsync(Guid containerId, string filePath, CancellationToken cancellationToken = default);
}
