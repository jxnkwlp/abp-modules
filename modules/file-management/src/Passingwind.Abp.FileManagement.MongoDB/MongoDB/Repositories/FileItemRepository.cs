...
public class FileItemRepository : MongoDbRepository<FileManagementMongoDbContext, FileItem, Guid>, IFileItemRepository
{
    public FileItemRepository(IMongoDbContextProvider<FileManagementMongoDbContext> dbContextProvider) : base(dbContextProvider)
    {
    }
    ...
    public async Task<FileItem?> FindByPathAsync(Guid containerId, string filePath, CancellationToken cancellationToken = default)
    {
        ...
        return await query.FirstOrDefaultAsync(x => x.ContainerId == containerId && x.Path != null && x.Path.FullPath == filePath, cancellationToken: cancellationToken);
    }
    ...
    public async Task<FileItem> GetByPathAsync(Guid containerId, string filePath, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await query.FirstOrDefaultAsync(x => x.ContainerId == containerId && x.Path != null && x.Path.FullPath == filePath, cancellationToken: cancellationToken);
